...
def wordclasscification():
    ...
    with open('corpus/wordlabelcorpuslarge.txt') as fp:
        with open('corpus/wordneulabelsepe1','w') as file:
            for i in fp:
                # print i[:-1]
                try:
                    word=unicode(i[:-1])
                    upperline=0.016
                    floor=0.108 #0.01 0.013
                    upperlinefield=0.06
                    floorfield=0.02
                    upperlinepre=0.019
                    floorpre=0.018
                    try:
                        ...
                        if sub>upperline:
                            ...
                            modellabel=0
                        sub=(modelfield.similarity(word,u"好")+modelfield.similarity(word,u"快乐")+model.similarity(word,u"开心"))/3.0-(modelfield.similarity(word,u"坏")+modelfield.similarity(word,u"悲伤"))/2.0
                        if sub>upperlinefield:
                            ...
                            modelfieldlabel=0
                        sub= (modelpre.similarity(word,u"好")+modelpre.similarity(word,u"快乐")+model.similarity(word,u"开心"))/3.0-(modelpre.similarity(word,u"坏")+modelpre.similarity(word,u"悲伤"))/2.0
                        if sub>upperlinepre:
                            modelprelabel=1
                        elif sub<floorpre:
                            modelprelabel=-1
